...
private extension Workflow {
    func didCompleteIfNotYet() {
        // Since a workflow may be forked to produce multiple subscribed reactive chains, we should
        // ensure the didComplete method is only invoked once per Workflow instance. See `Step.commit`
        // on why the side-effects must be added at the end of the reactive chains.
        guard didInvokeComplete == false else { return }
...
public extension Step {
    ...
    @discardableResult
    final func commit() -> Workflow<WorkflowActionableItemType> {
    ...
    final func asPublisher() -> AnyPublisher<(ActionableItemType, ValueType), Error> {
        publisher
    }
}
